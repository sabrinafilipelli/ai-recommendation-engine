	...
	<div>
		<div class="mb-8">
			<div class="mb-4 font-semibold text-lg">
				What kind of cinematic experience are you looking for?
			</div>
			<div class="flex items-center">
				{#each cinemaTypes as type (type.value)}
					<button
						...
						class={` ${
							cinemaType === type.value ? 'bg-pink-500' : ''
						} text-slate-200 font-bold mr-2 text-sm mt-2 py-2 px-4 rounded-full border border-pink-600 hover:bg-pink-600 cursor-pointer`}
					>
						{type.title}
					</button>
				{/each}
		...
		<div>
			...
			<div class="flex items-center flex-wrap">
				{#each categoryTypes as category}
					<label
						class={`${
							selectedCategories.includes(category) ? 'bg-pink-500' : ''
						} text-slate-200 font-bold mr-2 mt-2 text-sm py-2 px-4 rounded-full border border-pink-600 hover:bg-pink-600 cursor-pointer`}
					>
						<input
							class="hidden"
							type="checkbox"
		...
		<div class="mt-8">
			...
			<button
				...
				class={`${
					loading
						? 'bg-pink-400/50'
						: 'bg-pink-500 hover:bg-gradient-to-r from-pink-700 via-pink-600 to-pink-700 '
				} mt-4 w-full h-10 text-white font-bold p-3 rounded-full flex items-center justify-center cursor-pointer`}
			>
				{#if loading}
					<LoadingIndicator />
				{:else}
